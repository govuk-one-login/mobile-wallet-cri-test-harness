...
describe("validatePreAuthorizedCode", () => {
  ...
  it("should return 'true' when pre-authorized code is valid", async () => {
    const preAuthorizedCode = await getTestJwt(authServerUrl, criUrl, clientId, kid)
  ...
  it("should throw 'HEADER_DECODING_ERROR' error when token header cannot be decoded", async () => {
    const preAuthorizedCode = "invalidHeader" + (await getTestJwt(authServerUrl, criUrl, clientId, kid))
    await expect(
  ...
  it("should throw 'INVALID_HEADER' error when header is missing 'kid' claim", async () => {
    const preAuthorizedCode = await getTestJwt(authServerUrl, criUrl, clientId, undefined)
    await expect(
      validatePreAuthorizedCode(preAuthorizedCode, jwks, criUrl, authServerUrl, clientId),
  ...
  it("should throw 'JWK_NOT_IN_DID' error when 'kid' claim does not match JWK 'kid'", async () => {
    const preAuthorizedCode = await getTestJwt(authServerUrl, criUrl, clientId, "not-the-same-kid")
    await expect(
      validatePreAuthorizedCode(preAuthorizedCode, jwks, criUrl, authServerUrl, clientId),
  ...
  it("should throw 'INVALID_PAYLOAD' error when payload is missing 'iss' claim", async () => {
    const preAuthorizedCode = await getTestJwt(authServerUrl, undefined, clientId, kid)
    await expect(
      validatePreAuthorizedCode(preAuthorizedCode, jwks, criUrl, authServerUrl, clientId),
  ...
  it("should throw 'INVALID_PAYLOAD' error when token expiry is not 5 minutes", async () => {
    const preAuthorizedCode = await getTestJwt(authServerUrl, criUrl, clientId, kid, "10minutes")
  ...
  it("should throw 'INVALID_PAYLOAD' error when token issuer is not 'https://test-example-cri.gov.uk'", async () => {
    const preAuthorizedCode = await getTestJwt(authServerUrl, "https://different-issuer.gov.uk", clientId, kid)
  ...
  it("should throw 'INVALID_PAYLOAD' error when token audience is not 'https://test-auth-server.gov.uk'", async () => {
    const preAuthorizedCode = await getTestJwt("https://different-audience.gov.uk", criUrl, clientId, kid)
  ...
  it("should throw 'INVALID_PAYLOAD' error when token clientId is not 'TEST_CLIENT_ID", async () => {
    const preAuthorizedCode = await getTestJwt(authServerUrl, criUrl, 'invalid-client-id', kid)
...
async function getTestJwt(audience, issuer, clientId, kid, exp = "5minutes") {
  ...
  return await new SignJWT({ clientId: clientId, credential_identifiers: ["727da4d1-0636-4951-81eb-801c1cf90dd3"] })
