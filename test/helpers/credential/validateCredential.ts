...
import Ajv from "ajv";
import { headerSchema } from "../preAuthorizedCode/headerSchema";
...
export async function validateCredential(
  ...
  const verifyResult = await verifySignature(jwks, header, credential);

  validatePayload(verifyResult, proofJwt);
...
function validatePayload(
  ...
  const didKey = decodeProtectedHeader(proofJwt).kid;
